...
The planned `0.4.0-alpha.5` contract replaces the older independent-footer-only frontend description, not the underlying route architecture:

- **Surface ownership:** when an existing configured canonical `github-copilot` provider card is mounted, embed login/status/**Refresh models**/**Manage** there and suppress the separate footer account controller. If no such row is mounted, keep footer fallback usable; old Core retains its settings section. The shared account-state owner, including its in-flight authorization state, survives transfer only while another eligible surface remains mounted. Last-surface unmount or nonoverlapping declaration replacement stops polling. A later controller starts with a read-only status request; a sign-in completed outside that controller does not trigger automatic discovery, so use explicit **Refresh models**.
- **Discovery trigger:** exactly one bounded discovery follows this UI user's successful **Start sign-in**, whether success returns immediately or is observed through polling. Initial status for an already-signed-in account, view opening, repeated status reads and details toggles stay network-free. The status RPC itself remains read-only; the successful UI flow initiates discovery separately. Explicit **Refresh models** remains available; discovery errors do not authorize automatic retry loops, repeated login or silent model selection.
...
- **Evidence still required:** verify mounted existing-card embedding and footer suppression, no-row and old-Core fallback, shared-owner transfer during sign-in/discovery while another eligible surface remains mounted, polling cleanup on last-surface unmount/nonoverlapping declaration replacement, read-only re-entry without auto-discovery of external completion, immediate and polled success exactly once, network-free initial signed-in views/details, visible refresh errors, and retained native controls. Read the implementation PR's actual test/browser results before claiming these checks passed. This documentation change is not runtime or live OAuth evidence.
- **Media provenance:** `docs/images/copilot-provider-entry.png` and `docs/images/copilot-provider-authorization.png` show the actual `0.4.0-alpha.5` built Client in network-disabled isolated Edge, using synthetic Remote responses and a surrounding provider-shell fixture with retained **Edit/Delete** controls. These are current provider-integrated synthetic UI screenshots, not live Core activation or production authorization evidence. The older `github-copilot-auth-flow.gif`, `copilot-auth-card-signed-in.png` and `copilot-device-code-copy.png` remain historical `0.4.0-alpha.3` isolated compact-account illustrations, not evidence of the current layout or post-sign-in discovery flow.
