# Migrate to the single account-discovered Copilot route

[English guide](../README.md) · [中文操作说明](#中文操作说明)

## Result and boundaries

New installations use **GitHub Copilot**, with the stable actual route ID `github-copilot-preview`. The unified account card provides sign-in and explicit account-model refresh without a native provider row. GPT-6 and other new IDs follow validated account metadata, not a static model allowlist.
...
1. **Run the updated plugin first.** Confirm the active profile has actually loaded the single-route implementation targeted by `0.4.0-alpha.2`, with matching Host and Client artifacts. Installed-on-disk is not loaded-runtime evidence. Do not remove the profile while an older plugin is running: older versions may recreate it during login, startup or authentication refresh. Use a published, checksum-verified release when available; a planned version number is not proof of publication.
2. **Verify account discovery.** In the unified **GitHub Copilot** card, confirm sign-in, click **Refresh account models**, and inspect accepted models, rejected IDs and capability warnings. Confirm the intended model exists on `github-copilot-preview`. Metadata acceptance is not a successful transport test. An optional explicit test request needs the user's approval and consumes the provider's normal resources; report its result separately. Do not repeatedly sign in or disable validation to force a missing model.
...
新安装只使用显示为 **GitHub Copilot** 的账号发现路由，真实 ID 保持 `github-copilot-preview`。统一卡片独立提供登录和 **Refresh account models**，不需要先添加原生 provider。GPT-6 等新模型依据经过校验的账号元数据接入，不依赖静态 ID 白名单；失败时显示诊断，不回退静态目录。
...
1. **先实际运行新版本。** 确认目标 `0.4.0-alpha.2` 的单路由实现已在活动 profile 加载，Host 与 Client 匹配。使用已发布且校验通过的制品；计划版本和磁盘安装都不是已发布／已加载的证明。旧版本可能在启动、登录或认证刷新时重建被删 profile，不能先删除再期待旧进程保留缺失。
2. **验证发现结果。** 在统一卡片确认登录，主动点击 **Refresh account models**，检查目标模型是否在 `github-copilot-preview` 的接受列表及能力警告中。元数据成功不等于真实调用成功；可选测试请求须明确批准，正常消耗供应方资源，结果单独报告。不要重复登录或关闭校验强行恢复模型。
