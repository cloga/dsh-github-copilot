...
- 新 profile 会将账号 credential 中的 `availableModelIds` 与当前安装的 pi-ai catalog 取交集；状态刷新也会根据现有 grant 自愈缺失、空或不完整的 profile。每个模型都会写入 catalog `api`，从而在不写 route 级 `api`、`baseURL` 或 `apiKeyEnv` 的情况下保留混合协议。
