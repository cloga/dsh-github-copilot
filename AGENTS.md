...
| Metadata freshness | `github-copilot.accountModelTtlMs` defaults to 86400000 (24h maximum reuse); `accountModelFailureCooldownMs` defaults to 300000 (5min non-forcing failure cooldown); shared Host single flight, no periodic timers |
...
| README GIF/PNG illustrations | Primary alpha.6 model-freshness/model-refreshing PNGs show the built Client in isolated Edge with synthetic Remote/provider shell; browser checks cover stale counts, fresh reopen, Manage refresh, Retry, credentials, login force and 375px without external network/browser errors. Host TTL/cooldown timing needs unit tests, not screenshots; no live Core/production authorization. Provider-entry/authorization PNGs remain historical alpha.5; older compact GIF/PNGs alpha.3 |
