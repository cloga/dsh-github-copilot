...
Prepare version alignment in the implementation PR when practical. Use an unused aligned prepared version or prepare the next appropriate version through a PR; retain the current prerelease channel unless promotion is requested. A release change keeps `package.json`, `deployment-baseline.json`, both README URLs and a fresh annotated `v<version>` tag aligned. Never move or reuse a tag. Local installation, sign-out and worktree checkout still need explicit approval; restarting active Sessions requires acknowledgement of the interruption.
