...
describe('compact account lifecycle', () => {
  it('joins manual retries to an outstanding authorization poll', async () => {
    vi.useFakeTimers()
    const h = await harness(pending), wait = deferred<ReturnType<typeof ok>>()
    h.api.status.mockReturnValueOnce(wait.promise)
    await vi.advanceTimersByTimeAsync(500)
    const retry = h.account.retryStatus()
    for (let index = 0; index < 100; index++) void h.account.retryStatus()
    await flush()
    expect(h.api.status).toHaveBeenCalledTimes(2)
    wait.resolve(ok(discovered))
    await retry
    expect(h.account.getSnapshot().view).toEqual(discovered)
    expect(vi.getTimerCount()).toBe(0)
  })
  it('waits for a detached read before fetching a fresh lifetime snapshot', async () => {
    const api = remote(), first = deferred<ReturnType<typeof ok>>()
    api.status.mockReturnValueOnce(first.promise)
    ...
    first.resolve(ok(signedOut))
    await flush()
    expect(api.status).toHaveBeenCalledTimes(2)
    expect(account.getSnapshot().view).toEqual(discovered)
  })
  it('does not send a queued lifetime read after that lifetime is detached', async () => {
    const api = remote(), first = deferred<ReturnType<typeof ok>>()
    api.status.mockReturnValueOnce(first.promise)
    const account = createCompactAccount(api, authorizationViewFrom, async () => {})
    const detach = account.attach()
    await flush()
    detach()
    const detachAgain = account.attach()
    detachAgain()
    first.resolve(ok(signedOut))
    await flush()
    expect(api.status).toHaveBeenCalledOnce()
  })
  it('bounds pending authorization to 32 status reads in the first minute', async () => {
    vi.useFakeTimers()
    const h = await harness(pending)
    await vi.advanceTimersByTimeAsync(60_000)
    expect(h.api.status).toHaveBeenCalledTimes(32)
    expect(vi.getTimerCount()).toBe(1)
    await h.account.cancel()
    await vi.advanceTimersByTimeAsync(60_000)
    expect(h.api.status).toHaveBeenCalledTimes(32)
    await h.account.start()
    h.api.status.mockResolvedValueOnce(ok(discovered))
    await vi.advanceTimersByTimeAsync(500)
    expect(h.account.getSnapshot().view).toEqual(discovered)
    expect(h.api.discoverModels).toHaveBeenCalledOnce()
    expect(vi.getTimerCount()).toBe(0)
  })
  it('stops polling after transport failure without clearing the last authorization state', async () => {
    vi.useFakeTimers()
    const h = await harness(pending)
    h.api.status.mockRejectedValueOnce(new Error('PRIVATE_TRANSPORT_ERROR'))
    await vi.advanceTimersByTimeAsync(500)
    expect(h.account.getSnapshot().error).toBe('COPILOT_AUTHORIZATION_STATUS_FAILED')
    expect(h.account.getSnapshot().view?.phase).toBe('authorizing')
    await vi.advanceTimersByTimeAsync(60_000)
    expect(h.api.status).toHaveBeenCalledTimes(2)
    expect(vi.getTimerCount()).toBe(0)
  ...
  it('coalesces retries and invalidations while retaining one initial freshness check', async () => {
    const api = remote(signedIn), first = deferred<ReturnType<typeof ok>>()
    api.status.mockReturnValueOnce(first.promise)
    const account = createCompactAccount(api, authorizationViewFrom, async () => {})
    cleanups.push(account.attach())
    await flush()
    for (let index = 0; index < 100; index++) void account.retryStatus()
    first.resolve(ok(signedIn))
    await flush()
    expect(api.status).toHaveBeenCalledOnce()
    expect(api.ensureModels).toHaveBeenCalledOnce()
    const next = deferred<ReturnType<typeof ok>>()
    api.status.mockReturnValueOnce(next.promise)
    const retry = account.retryStatus()
    await flush()
    for (let index = 0; index < 100; index++) {
      account.invalidate()
      void account.retryStatus()
    }
    expect(account.getSnapshot().view).toBeUndefined()
    expect(api.status).toHaveBeenCalledTimes(2)
    api.status.mockResolvedValueOnce(ok(signedOut))
    next.resolve(ok(discovered))
    await retry
