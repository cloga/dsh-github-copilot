...
async function mount(remote: DualModelRemote, options: Omit<DualModelCardProps, 'remote'> = {}) {
  ...
  const render = async (next: DualModelRemote = currentRemote, props = currentProps) => {
    currentRemote = next
    currentProps = props
    await act(async () => { root!.render(createElement(DualModelCard, { remote: next, ...props })) })
  }
  const currentWorkspace = async (currentWorkspaceId?: string) => {
    await render(currentRemote, { ...currentProps, currentWorkspaceId })
  }
  const unmount = () => { root?.unmount(); root = undefined }
  cleanups.push(unmount)
  await render()
  return { container, render, unmount, currentWorkspace,
...
describe('scoped Copilot dual-model card in the real React DOM', () => {
  it.each([true, false])('pairs theme foregrounds with opaque select and option surfaces (writable=%s)', async writable => {
    ...
    for (const name of ['planner', 'executor']) {
  ...
  it('keeps unavailable saved model options themed and never substitutes an unavailable current workspace', async () => {
    const configuration = { ...enabled, plannerModel: 'retired-model' }
    const remote = remotes(view({ configuration }))
    const card = await mount(remote, { currentWorkspaceId: 'project-b' })
    remote.view.mockResolvedValueOnce(ok(view({ configuration, workspaces: [{ id: 'project-a', name: 'Project A' }] })))
    await click(card, 'reload')
    const control = card.field<HTMLSelectElement>('planner')
    expect(control.value).toBe('retired-model')
    const option = Array.from(control.options).find(item => item.value === 'retired-model')!
    expect(option.disabled).toBe(true)
    expect(option.style.backgroundColor).toBe('var(--dsw-alias-bg-layer-1, Canvas)')
    expect(option.style.color).toBe('var(--dsw-alias-label-secondary, GrayText)')
    expect(card.field('workspace').tagName).not.toBe('SELECT')
    expect(card.field('workspace').dataset.workspaceId).toBe('project-b')
    expect(card.field('workspace').textContent).toContain('current workspace is unavailable')
    expect(card.field('workspace').textContent).not.toContain('Project A')
    expect(remote.save).not.toHaveBeenCalled()
    expect(remote.create).not.toHaveBeenCalled()
  })
  it.each([['en-US', 'Model roles', 'Save configuration'], ['zh-CN', '模型分工', '保存配置']] as const)(
    'renders localized, labeled controls and defaults off: %s', async (locale, title, save) => {
      ...
      for (const name of ['enabled', 'planner', 'executor']) {
        const control = card.field(name)
        expect(control.id).not.toBe('')
        expect(card.container.querySelector(`label[for="${control.id}"]`)).not.toBeNull()
      }
      const destination = card.field('workspace')
      expect(destination.tagName).not.toBe('SELECT')
      expect(document.getElementById(destination.getAttribute('aria-labelledby')!)?.textContent)
        .toBe(locale === 'en-US' ? 'New session destination' : '新会话目标工作区')
      expect(card.container.querySelectorAll('select')).toHaveLength(2)
      expect(card.text()).toContain(locale === 'en-US' ? 'profile-global' : '整个配置档案')
      expect(card.text()).toContain(locale === 'en-US' ? 'explicitly create' : '显式新建')
  ...
  it('requires two catalog models, saves with CAS, and creates only the saved configuration', async () => {
    ...
    expect(card.field('save').disabled).toBe(true)
    await select(card, 'executor', 'code-model')
    await card.currentWorkspace('project-b')
    expect(card.field('create').disabled).toBe(true)
  ...
  it('saves profile-global roles without a current workspace or accessible workspaces', async () => {
    const remote = remotes(view({ workspaces: [] }))
    const card = await mount(remote)
  ...
  it('inherits the externally supplied current workspace without a picker or automatic creation', async () => {
    const remote = remotes(view({ configuration: enabled }))
    const card = await mount(remote, { currentWorkspaceId: 'project-b' })
    expect(card.container.querySelector('select[data-dsh-dual-model-workspace]')).toBeNull()
    expect(card.field('workspace').textContent).toBe('Project B')
    expect(card.field('workspace').dataset.workspaceId).toBe('project-b')
    expect(card.field('create').disabled).toBe(false)
    expect(remote.create).not.toHaveBeenCalled()
    expect(card.field('save').parentElement).toBe(card.field('reload').parentElement)
    expect(card.field('create').parentElement).not.toBe(card.field('save').parentElement)
    expect(card.container.querySelectorAll('section')).toHaveLength(1)
    await click(card, 'create')
    expect(remote.create).toHaveBeenCalledExactlyOnceWith({ requestId: expect.any(String), workspaceId: 'project-b', expectedRevision: 4 })
  })

  it('uses a changed current workspace before creation without reloading or discarding edits', async () => {
    const remote = remotes()
    const card = await mount(remote, { currentWorkspaceId: 'project-a' })
    await configure(card)
    await card.currentWorkspace('project-b')
    expect(card.field<HTMLSelectElement>('planner').value).toBe('plan-model')
    expect(card.text()).toContain('Save your changes')
    expect(card.field('workspace').textContent).toBe('Project B')
    expect(remote.view).toHaveBeenCalledTimes(1)
    await click(card, 'save')
    await click(card, 'create')
    expect(remote.create.mock.calls[0]![0]).toMatchObject({ workspaceId: 'project-b', expectedRevision: 5 })
  })

  it.each([undefined, '', 'unknown-project'])('never substitutes a missing or unknown current workspace: %s', async currentWorkspaceId => {
    const remote = remotes(view({ configuration: enabled }))
    const card = await mount(remote, { currentWorkspaceId })
    expect(card.field('create').disabled).toBe(true)
    expect(card.field('save').disabled).toBe(false)
    expect(card.field('workspace').textContent).toContain(currentWorkspaceId ? 'current workspace is unavailable' : 'No current workspace')
    expect(card.field('workspace').textContent).not.toContain('Project A')
    expect(card.field('workspace').textContent).not.toContain('Project B')
    await click(card, 'create')
    expect(remote.create).not.toHaveBeenCalled()
    await card.currentWorkspace('project-b')
    expect(card.field('create').disabled).toBe(false)
    await card.currentWorkspace(currentWorkspaceId)
    expect(card.field('create').disabled).toBe(true)
    expect(remote.view).toHaveBeenCalledTimes(1)
  })

  it('blocks a new creation when the current workspace disappears from the Host view, without blocking Save', async () => {
    const remote = remotes(view({ configuration: enabled }))
    const card = await mount(remote, { currentWorkspaceId: 'project-b' })
    expect(card.field('create').disabled).toBe(false)
    remote.view.mockResolvedValueOnce(ok(view({ configuration: enabled, workspaces: [{ id: 'project-a', name: 'Project A' }] })))
    await click(card, 'reload')
    expect(card.field('workspace').dataset.workspaceId).toBe('project-b')
    expect(card.field('workspace').textContent).toContain('current workspace is unavailable')
    expect(card.field('workspace').textContent).not.toContain('Project A')
  ...
  it.each([undefined, 'project-b'])('retains the original pending destination despite current workspace change or disappearance: %s', async currentWorkspaceId => {
    const remote = remotes(view({ configuration: enabled }))
    remote.create.mockResolvedValueOnce(failure()).mockResolvedValueOnce(failure())
    const card = await mount(remote, { currentWorkspaceId: 'project-a' })
    await click(card, 'create')
    const original = remote.create.mock.calls[0]![0]
    await card.currentWorkspace(currentWorkspaceId)
    expect(card.field('workspace').dataset.workspaceId).toBe('project-a')
    expect(card.field('workspace').textContent).toBe('Project A')
    expect(card.text()).toContain('Original destination')
    expect(card.field('create').disabled).toBe(false)
    ...
    remote.view.mockResolvedValueOnce(ok(view({ revision: 99, configuration: enabled, workspaces: [{ id: 'project-b', name: 'Project B' }] })))
    const remounted = await mount(remote, { currentWorkspaceId })
    expect(remounted.field('workspace').dataset.workspaceId).toBe('project-a')
    expect(remounted.field('workspace').textContent).toContain('project-a')
    expect(remounted.field('workspace').textContent).not.toContain('Project B')
    expect(remounted.text()).toContain('Original destination')
    expect(remote.create).toHaveBeenCalledTimes(2)
    await click(remounted, 'create')
    expect(remote.create.mock.calls[2]![0]).toEqual(original)
  })
  ...
  it('fails closed if models disappear after an explicit reload', async () => {
    const remote = remotes(view({ configuration: enabled }))
    const card = await mount(remote)
    await card.currentWorkspace('project-a')
    expect(card.field('create').disabled).toBe(false)
  ...
  it('retries an uncertain creation with the identical request, workspace and revision, never automatically', async () => {
    ...
    expect(card.text()).not.toContain('PRIVATE_')
    expect(card.field('workspace').dataset.workspaceId).toBe(original.workspaceId)
    expect(card.field('reload').disabled).toBe(true)
    expect(card.field('save').disabled).toBe(true)
    expect(remote.create).toHaveBeenCalledTimes(1)
    await click(card, 'create')
  ...
  it('retains an uncertain request across unmount with the same Remote owner', async () => {
    ...
    await first.currentWorkspace('project-b')
    ...
    expect(remote.create).toHaveBeenCalledTimes(1)
    expect(second.field('workspace').dataset.workspaceId).toBe('project-b')
    expect(second.field('workspace').textContent).toBe('Project B')
  ...
  it.each([['en-US', 'Session created.', 'Open created session'], ['zh-CN', '会话已创建。', '打开已创建的会话']] as const)(
    ...
    const oldOpen = vi.fn()
    const first = await mount(remote, { onOpenSession: oldOpen })
    await first.currentWorkspace('project-a')
    await click(first, 'create')
    await act(async () => { first.unmount() })
  ...
  it('clears a known failure only with explicit Host proof that no session was created', async () => {
    ...
    expect(firstRequest.workspaceId).toBe('project-a')
    expect(remote.create.mock.calls[1]![0].workspaceId).toBe('project-b')
  })
})
