...
export async function prepareTaggedCoreFixture({ root, core, target, release }, { git = readGit, runner } = {}) {
  ...
  for (const path of [...tests.map(name => join(pluginRoot, name)), vitestCli, join(coreRoot, 'vitest.shared.ts')]) {
