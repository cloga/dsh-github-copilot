...
4. DSH polls only while authorization is in flight: the interval starts at 500 ms, increases to 1 second, then stays at 2 seconds. Each account controller merges overlapping status reads; failed status reads pause polling until **Retry status**. A new sign-in resets the interval. On success, the code, verification link and copy feedback disappear; the automatic authorization area closes and the compact row shows **Signed in**, **Refresh models**, and **Manage**. Manually opened management details stay open. Cancelling clears the old code; failures remain visible rather than disappearing into a collapsed area.
...
Status transport failures do not prove that GitHub authorization expired. Status retries never repeat sign-in, sign-out, discovery or repair automatically. The compact controller drains an obsolete pending status request before sending a fresh one; ignoring an old result does not cancel its network request. These plugin-local controls reduce request pressure without changing Core's request scheduling.
...
```sh
curl -LO https://github.com/cloga/dsh-github-copilot/releases/download/v0.4.0-alpha.3/dsh-github-copilot-0.4.0-alpha.3.tgz
curl -LO https://github.com/cloga/dsh-github-copilot/releases/download/v0.4.0-alpha.3/SHA256SUMS
...
$actual = (Get-FileHash .\dsh-github-copilot-0.4.0-alpha.3.tgz -Algorithm SHA256).Hash.ToLowerInvariant()
