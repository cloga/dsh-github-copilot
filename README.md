...
- Successful sign-in intersects the account's `availableModelIds` with the installed pi-ai catalog when creating a new route profile. Status refreshes also self-heal an absent, empty, or incomplete profile from an existing grant. Each model entry carries its catalog `api`, so one route preserves mixed protocols without route-level `api`, `baseURL`, or `apiKeyEnv`.
